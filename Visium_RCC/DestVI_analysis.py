...
VISIUM_RCC_DESTVI_ADATA_PATH = os.path.join(ADATA_PATH, 'Visium_RCC_DestVI_v2_adata.h5ad')
VISIUM_RCC_SC_MODEL_PATH = os.path.join(MODELS_PATH, 'Visium_RCC_DestVI_v2_sc')
VISIUM_RCC_ST_MODEL_PATH = os.path.join(MODELS_PATH, 'Visium_RCC_DestVI_v2_st')
...
st_adata = sc.read_h5ad(f"{ADATA_PATH}/ST_adata.h5ad")
...
sc_adata = sc.read_h5ad('/home/projects/nyosef/oier/Harreman_files/Slide_seq_lung/h5ads/Slide_seq_lung_sc_ref_adata.h5ad')
sc_adata = sc_adata[sc_adata.obs['organ__ontology_label'] == 'kidney'].copy()
...
if train_sc:
    ...
    sc_model.save(VISIUM_RCC_SC_MODEL_PATH, overwrite=True, save_anndata=True)
else:
    sc_model = scvi.model.CondSCVI.load(VISIUM_RCC_SC_MODEL_PATH)
...
if train_st:
    ...
    st_model.save(VISIUM_RCC_ST_MODEL_PATH, overwrite=True)
else:
    st_model = DestVI.load(VISIUM_RCC_ST_MODEL_PATH, st_adata)
...
st_adata.write(VISIUM_RCC_DESTVI_ADATA_PATH)

print('Loading AnnData...')
st_adata = sc.read_h5ad(VISIUM_RCC_DESTVI_ADATA_PATH)
